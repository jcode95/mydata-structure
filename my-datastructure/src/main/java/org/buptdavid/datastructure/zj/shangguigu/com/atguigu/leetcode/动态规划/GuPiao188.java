...
public class GuPiao188 {
     ...
     * <p>
     * <p>
     * dp[i][k][0]=max(dp[i-1][k][0]，dp[i-1][k-1][1]+prices[i]);  //dp[i][k][0] 的值代表第i天没有持有股票的利润
     * dp[i][k][1]=max(dp[i-1][k][1],dp[i-1][k-1][0]-prices[i]);  //dp[i][k][1] 的值代表第i天持有股票的利润
     * <p>
     * 本题 base case
     * dp[-1][k][0]=0   还没开始肯定没有利润
     * dp[-1][k][1]=Integer.MAXVALUE     还没开始就持有股票
     * dp[i][0][0]=0;   没有交易次数 利润为0
     * dp[i][0][1] = Integer.MAXVALUE; 不允许交易的情况下，是不可能持有股票的，⽤负⽆穷表⽰这种不可能。
     */
    ...
    public static void main(String[] args) {
        ...
        int i = new GuPiao188().maxProfit(1,arr);
